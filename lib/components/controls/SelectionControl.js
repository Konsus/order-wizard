...
class SelectionControl extends React.Component {
    ...
    initialValue() {
        ...
        if (token != null)
            value = this.context.form.getValue(token);
        return value;
    }
    ...
    onValueChange(state) {
        ...
        if (this.props.valueRef)
            this.props.valueRef(state.value);
        if (key)
            this.context.form.setValue(key, state.value);
    ...
    renderOption(option, index, active) {
        if (active)
            return this.renderActiveOption(option, index);
    }
    ;
    /**
